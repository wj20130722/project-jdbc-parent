...
import com.wangjie.jdbc.mybatis.model.SysPrivilege;
import com.wangjie.jdbc.mybatis.model.SysRole;
import com.wangjie.jdbc.mybatis.model.SysUser;
import com.wangjie.jdbc.mybatis.type.Enabled;
...
public class UserMapperTest extends BaseMapperTest {
	...
	public void testSelectRolesByUserId(){
		...
		try {
			...
			//还需要查询出来的角色信息带有当前用户信息
			//1.增加用户信息字段如增加SysRoleExtend扩展所需字段
			//2.将用户实体类作为属性加入 使用“user.属性名”
	...
	public void testSelectRolesByUserIdAndRoleEnabled(){
		...
		try {
			...
			//角色数量大于 0 个
			Assert.assertTrue(roleList.size() > 0);
			List<SysRole> roleList2 = userMapper.selectRolesByUserIdAndRoleEnabled(1L, Enabled.enabled);
			//结果不为空
			Assert.assertNotNull(roleList2);
			//角色数量大于 0 个
			Assert.assertTrue(roleList2.size() > 0);
			Assert.assertNotEquals(roleList,roleList2);
		} finally {
			//不要忘记关闭 sqlSession
			sqlSession.close();
		}
	...
	public void testInsert(){
		...
		try {
			...
			//#{createTime, jdbcType=TIMESTAMP} 可以修改成 #{createTime, jdbcType=DATE} 只存储日期部分
	...
	/**
	 * 懒加载演示，默认使用jdk对返回对象做代理支持cglib或者javassist
	 * 与spring框架结合使用的时候，只能在service层调用延迟加载属性。否则会因为sqlsession的关闭导致异常
	 */
	@Test
